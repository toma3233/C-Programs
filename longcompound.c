/*
 * Fully Functioning Long Compound program
 * -Takes in user inputted molecule and outputs total atomic weight as well as the name of every original element within the molecule in alphabetic order
 *
 * 
 * Homework completed by Tom Abraham
 * Answer to Homework 5, question 5
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int multiply_by = 0;
/* Array that holds the names of each element in the molecule*/
char names[50][50];
/* Temp array used for sorting purposes*/
char temp[50];
/* number of names stored in names array*/
int name_counter = 0;

/* Creating the structure "element" that will store the atomic weight, symbol, and name of every element in the list.*/
struct element{
    float atomic_weight;
    char symbol[3];
    char name[40];
...
int processInput(char **name){
    ...
    while ((temp = fscanf(fp, "%f\t%s\t%s",&list.arr[list_length].atomic_weight, &list.arr[list_length].symbol, &list.arr[list_length].name)) != EOF){        
...
float searchList(char *sym, int num){
    ...
    for(i = 0; i < list_length; i++){
        if(strcmp(list.arr[i].symbol, sym) == 0){
            ...
            /* IF Else statements work together ensure that more than one of the same element is not added to the names array */
            if(name_counter > 0){
                for(j = 0; j < name_counter; j++){
                    if(strcmp(names[j], list.arr[i].name) == 0){
                        foundName = 1;
                        break;
                    }
                }
                if(foundName == 0){
                    strcpy(names[name_counter], list.arr[i].name);
                    ++name_counter;
                }
                foundName = 0;
            }
...
/* Helper function used to sort the array of names in alphabetical order*/
void sort(){ 
    int i;  
    int j;
...
/* Helper function to print out names with grammatical correctness */
void printNames(){
    int j = 0;
    
    /*If there is only one name in the array*/
    if(name_counter == 1){
        printf("The element is %s\n", names[0]);
    }

    /*If there are only two names in the array*/
    else if(name_counter == 2){
        sort();
        printf("The elements are %s and %s\n", names[0], names[1]);
    }
    /*If there are 3 or more names in the array*/
    else{
        sort();
    ...
    /*Reset name_counter to zero after names have been printed */
    name_counter = 0;
        ...
        if (len == 0){
           ...
           running_total = 0; 
            ...
            if (total == 0.00){
                ...
                /* Printing names of elements after total atomic weight has been printed*/
                printNames();
